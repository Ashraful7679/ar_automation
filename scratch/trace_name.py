...
# Trace around line 45-48 where SAFA is
current = None
last_prefix = None
last_date = None
...
for i in range(44, 52):
    ...
    if date_match:
        ...
        if current and current.get('amounts') and current.get('inv_str'):
            print(f'  SAVED prev: inv={current["inv_str"]}, names={current["name_parts"]}')
        ...
        for p in parts[1:]:
            if INV_PREFIX_RE.match(p):
                ...
                print(f'  Amount found: {p}, _name_collected set to True')
            elif not (INV_PREFIX_RE.match(p) or p.isdigit() or '/' in p):
                if not current.get('_name_collected'):
                    current['name_parts'].append(p)
        
        print(f'  NEW: inv={current["inv_str"]}, names={current["name_parts"]}, _name_collected={current["_name_collected"]}')
        continue
    ...
    if current and current.get('amounts'):
        ...
        for p in parts:
            if not current.get('_name_collected'):
                current['name_parts'].append(p)
        print(f'    Added to names: {current["name_parts"]}')
